...
class GraphCollector(Node, Collector):
    ...
    def process_msg(self, msg):
        if msg.mtype == MsgTypes.Transition:
            ...
            print(dt.datetime.now().strftime("%H:%M:%S"),self.name,'recvfrom',msg.identity,'latency:',dt.datetime.now()-dt.datetime.fromtimestamp(msg.heartbeat.timestamp),'storeQlen:',len(self.store.builders['graph'].pending.keys()),'heartbeat',msg.heartbeat.identity)
